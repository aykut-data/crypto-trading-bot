...
def calculate_indicators_15m(df):
    """
    Calculate professional technical indicators using TA-Lib for 15m timeframe.
...
if __name__ == "__main__":
    ...
    # Load 15-minute data
    input_file = f"{data_dir}/doge_usdt_15m.csv"
    
    if os.path.exists(input_file):
        print("📊 Loading 15m DOGEUSDT data...")
        df = pd.read_csv(input_file)
        df['timestamp'] = pd.to_datetime(df['timestamp'])
        
        # Calculate indicators
        print("🔢 Calculating indicators for 15m timeframe...")
        df_indicators = calculate_indicators_15m(df)
        
        # Save to data folder
        output_file = f"{data_dir}/doge_usdt_15m_indicators.csv"
        df_indicators.to_csv(output_file, index=False)
        
        print("✅ 15m indicators saved:", output_file)
