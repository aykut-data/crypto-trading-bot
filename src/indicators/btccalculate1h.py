...
def calculate_indicators_1h(df):
    """
    Calculate professional technical indicators using TA-Lib for 1h timeframe.
...
if __name__ == "__main__":
    ...
    # Load 1-hour data
    input_file = f"{data_dir}/btc_usdt_1h.csv"
    
    if os.path.exists(input_file):
        print("📊 Loading 1h BTCUSDT data...")
        df = pd.read_csv(input_file)
        df['timestamp'] = pd.to_datetime(df['timestamp'])
        
        # Calculate indicators
        print("🔢 Calculating indicators for 1h timeframe...")
        df_indicators = calculate_indicators_1h(df)
        
        # Save to data folder
        output_file = f"{data_dir}/btc_usdt_1h_indicators.csv"
        df_indicators.to_csv(output_file, index=False)
        
        print("✅ 1h indicators saved:", output_file)
