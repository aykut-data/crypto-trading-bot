...
def fetch_4h_data(symbol="BTCUSDT", days=180):
    """
    Fetch 4-hour kline data for the specified symbol from Binance.
    """
    ...
    print(f"📡 Fetching 4h {symbol} data for the last {days} days...")
    ...
    while since < end_time:
        ...
        time.sleep(0.5) # Anti-ban rate limit delay
...
def save_to_csv(klines, full_path):
    ...
    # Ensure the target data directory exists dynamically
...
if __name__ == "__main__":
    # 🎯 Define dynamic target path
    data_dir = os.path.join(base_dir, 'data')
    target_file = os.path.join(data_dir, "btc_usdt_4h.csv")
    
    # Fetch 180 days of 4-hour data
    raw_data = fetch_4h_data("BTCUSDT", days=180)
