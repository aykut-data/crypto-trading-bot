...
def fetch_1h_data(symbol="BTCUSDT", days=180):
    """
    Fetch 1-hour kline data for specified symbol (180 days = ~6 months).
    ...
    print(f"📡 Fetching 1h {symbol} data ({days} days = ~{days*24} candles)...")
    ...
    print(f"✅ Completed! {len(all_klines)} 1h {symbol} candles fetched.")  
    return all_klines

def save_to_csv(klines, filename="btc_usdt_1h.csv"):
    """
...
if __name__ == "__main__":
    klines = fetch_1h_data("BTCUSDT", days=180)
    save_to_csv(klines, "btc_usdt_1h.csv")
